...
def plot_2d_space(A: np.ndarray, b: np.ndarray, X: np.ndarray, Y: np.ndarray,
    ...
    # Iterate over each half-plane (one per row of A)
    for i in range(A.shape[0]):
        ...
        Z = np.where(dot_product.reshape(X.shape) > b[i], 0, Z)  # Reshape before comparison
...
def plot_1d_space(A: np.ndarray, b: np.ndarray, label: str, cmap: str, ax,
                  x_range: list)-> None:
    """
    Plots a 1D region (line) defined by the intersection of half spaces.
...
def plot_half_spaces(ab_pairs: list, num_of_iterations: int, ax,
                     x_range: list, y_range: list) -> None:
    """
    Plots the intersection of multiple sets of half-spaces defined by ab_pairs,
    where each pair consists of A (normals) and b (offsets) such that A*x <= b.
    ab_pairs is of the form [('label'_i, 'cmap_i', A_i, b_i), (...), ...]

    Args:
        ab_pairs: List of tuples containing ('label', 'cmap', A, b).
    ...
    try:
        ...
        for label, cmap, A, b in ab_pairs:
            # Distinguish cases based on the rank of A
            rank = np.linalg.matrix_rank(A)
            # 1d case (line)
            if rank == 1:
                plot_1d_space(A, b, label, cmap, ax, x_range)
            # 2d case
            elif rank == 2:
                plot_2d_space(A, b, X, Y, label, cmap, ax)
