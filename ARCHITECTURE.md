...
The LTI solvers compare quantities that are zero in exact arithmetic, a drift, a slack, an increment, against a rounding floor of 64 eps times the magnitudes that were summed to form them, never against an absolute constant: the problem's scale and coordinates no constraint touches must not change a decision. The closed form is used only while the condition number of `I - A_m` is below `1e8`; beyond it an episode is stepped or deflated. A frozen stall means a state fixed to rounding. The KKT certificate accepts a point only if it is feasible and complementary at rounding level and its stationarity residual, which bounds its distance to the projection, is within `1e-9` of the step from `z` beyond the rounding of `z`, the point and the multiplier terms. A `"kkt"` result is therefore the projection to that accuracy; rounding limits it when the tight normals are nearly dependent, since their multipliers are then large, and when the step is small beside the coordinates of `z` and the point, as for a starting point close to constraints far from the origin. A `"finality"` result is weaker: a drift below rounding counts as zero, so a drain slower than rounding can make the active set look final when it is not.
