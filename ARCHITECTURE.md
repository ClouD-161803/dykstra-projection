...
- `DykstraStallDetectionSolver` is this project's own contribution. Inside the cycle it flags stalling when a half-space is active and the iterate did not move between the previous cycle and this one at the same constraint index. `_handle_stalling` then computes, over all constraints, how many identical cycles would pass before some constraint becomes satisfied, and advances every correction vector by that many cycles at once. The intended guarantee is that the jump lands on the state the unmodified algorithm would have reached after those cycles, so only the path is shortened and the limit is untouched; the repository does not currently prove this, and a test that pins a fast-forwarded run against the plain solver's state at the same cycle count would be the right way to. It prints when it detects a stall and when it fast-forwards.
...
`tests/test_solver_regressions.py` covers behaviours that were once wrong. Constraints already satisfied at the start are retained; the dimension is inferred from the point; a mismatched explicit `dimensions` is rejected; zero iterations still records the initial error. New bug fixes belong here, test first.
